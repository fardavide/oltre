...
sealed interface FacilityActionUiState {
    ...
    data class AffordableIn(val label: TextRes) : FacilityActionUiState
    data class Locked(val reason: TextRes) : FacilityActionUiState
...
private fun FacilityRowUiState.sheetHeading(): TextRes = when (val current = action) {
    ...
    // `TextRes("")` rather than a catalogue entry, and it is the honest one: there is no message
    // here, so there is nothing for a language to say. A row at its ceiling has no verdict.
    -> verdict?.let { if (detail.ladder.isEmpty()) it.label else it.compactLabel } ?: TextRes("")
}
...
internal fun FacilityActionUiState.Upgrading.becomes(): TextRes =
