...
distilled `.ai/docs/brief.md` — the Notion page remains the design source of truth.
...
`.ai/tools/gradle-without-agp.sh` does that: it generates a minimal overlay for the three build
...
`.ai/rules/session-roles.md` says a cloud session may not work on **"anything a player sees. No
...
`galaxy-identity-sheet.md` is the design and `.ai/prompts/design-galaxy-identity.md` is the
...
`.ai/tools/gradle-without-agp.sh` swaps in a minimal overlay for the build files it covers. Two
