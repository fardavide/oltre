...
Mirror the Oltre-specific half into `.ai/skills/test-coverage/SKILL.md` under the behaviour
...
- **Inject the clock** — add a `now: () -> Instant = { Clock.System.now() }` parameter to `App`,
  ...
  to `.ai/docs/decisions.md` in the same PR, with the rejected alternative.
...
- Update `.ai/docs/status.md` (the "no behaviour test touches anything" entry under *Pending*
