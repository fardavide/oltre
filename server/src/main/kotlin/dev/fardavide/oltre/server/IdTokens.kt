...
// **The half of verification that is the same whatever the token is *for*.** An ID token and one of
// Apple's server-to-server notifications are signed the same way, by the same keys, published at the
// same URL — and they carry entirely different claims, which is why the two are separate steps rather
// than one function with a flag. Sharing the signature is what makes `#111`'s notification endpoint
// a route and a claims rule rather than a second copy of the thing worth being most careful about.
internal sealed interface Signed {

    data class Claims(val claims: JWTClaimsSet) : Signed

    data class Refused(val reason: String) : Signed
}

// The order is deliberate and each step is only reached because the one before it held. Algorithm
// before key, so a token that is not eligible costs no round trip; signature before claims, so that a
// tampered token is refused as tampered rather than as whatever its rewritten claims happen to say.
internal suspend fun signedClaims(keys: JwksKeys, spec: ProviderSpec, token: String): Signed {
    val jwt = try {
        SignedJWT.parse(token)
    } catch (e: ParseException) {
        return Signed.Refused("not a signed token: ${e.message}")
    }

    if (jwt.header.algorithm != ACCEPTED_ALGORITHM) {
        return Signed.Refused("signed with ${jwt.header.algorithm} rather than $ACCEPTED_ALGORITHM")
    }
    val keyId = jwt.header.keyID?.takeIf { it.isNotBlank() }
        ?: return Signed.Refused("no key id: nothing says which key to look up")
    val key = keys.keyFor(spec.jwksUri, keyId)
        ?: return Signed.Refused("$keyId is not a key ${spec.provider.providerName.value} publishes")

    // `verify` answers false for a signature that does not match and throws for one it cannot even
    // attempt. Both are the same answer to a caller: this token was not signed by the key it names.
    val verified = try {
        jwt.verify(RSASSAVerifier(key))
    } catch (e: JOSEException) {
        return Signed.Refused("the signature could not be checked: ${e.message}")
    }
    if (!verified) return Signed.Refused("the signature does not match $keyId")

    // Read after the signature held, and not before. A claims set that will not parse in a token
    // whose signature *did* is a provider bug rather than an attack, but reading it first would mean
    // deciding things about a body nobody has vouched for yet.
    return try {
        Signed.Claims(jwt.jwtClaimsSet)
    } catch (e: ParseException) {
        Signed.Refused("the claims could not be read: ${e.message}")
    }
}
...
internal class IdTokenVerifier(
    ...
    suspend fun verify(provider: IdentityProvider, token: IdToken, nonce: SignInNonce): TokenVerdict {
        ...
        return when (val read = signedClaims(keys, spec, token.value)) {
            is Signed.Refused -> TokenVerdict.Refused(read.reason)
            is Signed.Claims -> read.claims.judge(spec, nonce)
        }
    }
