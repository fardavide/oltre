...
//
// `pendingWork` is null once a tab has a real screen behind it — today only the Colony does, and
// this table is the honest record of that. The strings are **placeholder copy**: what a screen
// says to the player is content, and content is Davide's. They say the one true thing an unbuilt
// tab can, which is what will be there.
enum class OltreTab(val label: String, val pendingWork: String?) {
    COLONY(label = "Colony", pendingWork = null),
    RESEARCH(label = "Research", pendingWork = "The tech tree lands here."),
    SHIPYARD(label = "Shipyard", pendingWork = "Ship construction lands here."),
    GALAXY(label = "Galaxy", pendingWork = "The star map lands here."),
