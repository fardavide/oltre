...
object Italian : Translations {
    ...
    override fun resolve(id: StringId, args: List<Arg>): String = when (id) {
        ...
        StringId.AllianceAsking -> "Sto chiedendo al server."
        StringId.AllianceSearchLabel -> "Trovane una"
        StringId.AllianceSearchIdle -> "Scrivi un nome per trovare un'alleanza a cui unirti."
        StringId.AllianceSearchEmpty -> "Nessuna alleanza si chiama “${args.text(0)}”."
        StringId.AllianceSearchHeld ->
            "Una ricerca interroga il server. Senza rete non c'è nulla in cui cercare."
        StringId.AllianceRequestSeat -> "Chiedi"
        StringId.AllianceWithdraw -> "Ritira"
        StringId.AllianceFoundLabel -> "Fondane una tua"
        StringId.AllianceFoundBody -> "Un nome, una sigla e un prezzo che la rende una scelta e non una formalità."
        StringId.AllianceFoundName -> "Nome"
        StringId.AllianceFoundTag -> "Sigla"
        StringId.AllianceFoundTagRule -> "Tre o quattro maiuscole o cifre."
        StringId.AllianceFoundPrice -> "Fondarla costa ${args.text(0)}."
        StringId.AllianceFoundPriceUnread -> "Il prezzo non è ancora stato letto."
        StringId.AllianceFoundShort -> "Alla tua colonia non basta."
        StringId.AllianceFoundAction -> "Fondala"
        StringId.AllianceNameTaken -> "“${args.text(0)}” è già di qualcun altro."
        StringId.AllianceTagTaken -> "“${args.text(0)}” è già di qualcun altro."
        StringId.AllianceWaitingOn -> "In attesa di ${args.text(0)}."
        StringId.AllianceWaitingBody ->
            "La tua richiesta resta lì finché qualcuno non risponde. Non scade, e nessuno viene avvisato."
        StringId.AllianceSeatsLine -> "${args.number(0)} posti su ${args.number(1)}"
        StringId.AllianceLevelBadge -> "LV ${args.number(0)}"
        StringId.AlliancePendingLabel -> "In attesa"
        StringId.AlliancePendingEmpty -> "Non c'è nessuno in attesa. Una richiesta arriva quando qualcuno ti trova per nome."
        StringId.AllianceAccept -> "Accetta"
        StringId.AllianceDecline -> "Rifiuta"
        ...
        StringId.AllianceConfirmAllAction -> "Versa tutto"
        ...
        StringId.AllianceLeaveAction -> "Esci"
        StringId.AllianceDisbandAction -> "Sciogli"
        StringId.RefusedContributionLead -> "Non è arrivato al fondo."
        StringId.RefusedContributionBody ->
            "Un versamento non può aspettare la rete: l'alleanza deve confermare che ne fai ancora parte. " +
                "Non è uscito nulla dalla tua colonia."
