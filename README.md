...
| `protocol` | KMP, `core`'s target set. What the client and the server say to each other: the verbs as data, the sync envelope, the rejection taxonomy, the sign-in pair and the API version. Depends on `core` and nothing else, and holds no I/O — it does not know what a network is. |
| `sim` | JVM. Headless balancing harness, fast-forwards weeks in milliseconds. Never ships. |
| `client/*` | KMP + Compose Multiplatform: desktop, iOS, Android. Directory of modules — `:client:shell` (composition root, navigation and the resource rail), `:client:design:*` (tokens, icons, components, formatting), one directory per feature holding its layers: `colony`, `research`, `shipyard`, `galaxy` and `fleets` are each a `:ui` (the composables and the models they render) plus a `:presentation` (the mapping from `core` state into those models). Then `:client:save:data` (the JSON snapshot on disk), `:client:net:data` (the only module that opens a socket: the Ktor client, the outbox that keeps a verb tapped with no signal, and the sync loop that reconciles what the server made of it), `:client:notifications:data` (the local alerts that are the check-in loop), `:client:debug:*` (the shake-to-open debug menu), `:client:tilt:*` (which way the device is being held, for the sky behind every screen). |
| `server` | JVM + Ktor. The engine, over HTTP: `POST /v1/colony` founds a colony and mints its galaxy, `POST /v1/sync` replays whatever the client queued through the same `core` the phone runs and hands the authoritative colony back. The colony lives in three Postgres tables and survives a restart; two devices syncing at once are settled by a compare-and-set on the colony's version. A colony belongs to an account: `POST /v1/auth/apple` and `POST /v1/auth/google` verify a provider ID token against that provider's own key set and mint a session this server signs, `POST /v1/auth/refresh` renews it, and `DELETE /v1/account` deletes — cascading to the colony and every spent idempotency key. |
...
- [.ai/docs/brief.md](.ai/docs/brief.md) — distilled project brief; points to the Notion design page.
- [.ai/docs/balance-log.md](.ai/docs/balance-log.md) — every round of balance tuning: the numbers, the feedback that changed them, what is still open.
