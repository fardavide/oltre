...
def write_android(svg: str, foreground: str) -> None:
    ...
    (ANDROID_STAGE / "README.md").write_text(
        ...
        "Generated by `art/icon/generate.py`, and **copied** into `androidApp/src/main/res/` —\n"
        "the app module reads resources from its own source set, not from a directory beside\n"
        "the artwork.\n\n"
        "So regenerating is two steps: rerun the generator, then copy `mipmap-*/` and\n"
        "`values/ic_launcher_background.xml` over `androidApp/src/main/res/`. The manifest\n"
        "already points `android:icon`/`android:roundIcon` at `@mipmap/ic_launcher`.\n\n"
        ...
        "Not generated from here: the status-bar icon\n"
        "(`client/notifications/data/src/androidMain/res/drawable/ic_notification.xml`).\n"
        "Android masks a small icon to a flat silhouette, so it is a hand-authored reduction\n"
        "of the master's arc rather than a rendering of it.\n"
    )
