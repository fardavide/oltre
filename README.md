...
| `server` | JVM + Ktor. The engine, over HTTP: `POST /v1/colony` founds a colony and mints its galaxy, `POST /v1/sync` replays whatever the client queued through the same `core` the phone runs and hands the authoritative colony back. The colony lives in memory until persistence lands. |
