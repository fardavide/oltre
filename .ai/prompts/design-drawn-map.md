...
[issue #69](https://github.com/fardavide/oltre/issues/69) and `.ai/docs/galaxy-identity-sheet.md`.
...
- Write `.ai/docs/drawn-map-sheet.md` — the prose record of every call, in the repo's own voice,
  the way `galaxy-identity-sheet.md` §12 holds the fleet sheet's. Archive the returned `.dc.html`
  under `.ai/docs/design/` and add its row to that folder's README.
