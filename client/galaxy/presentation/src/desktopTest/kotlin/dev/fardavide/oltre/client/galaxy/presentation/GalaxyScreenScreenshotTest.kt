...
class GalaxyScreenScreenshotTest {
    ...
    fun `the home system at phone width`() {
        capture(width = 393, height = 760, uiState = homeSystemUiState, name = "galaxy_home_system")
    }
    ...
    fun `an unsurveyed system at phone width`() {
        capture(width = 393, height = 620, uiState = unsurveyedSystemUiState, name = "galaxy_unsurveyed")
    }

    @Test
    fun `every verdict at phone width`() {
        capture(width = 393, height = 800, uiState = everyVerdictUiState, name = "galaxy_every_verdict")
    }

    // 320dp is narrower than any phone and reachable since the app became a real iPad app. The
    // blocked cards wrap and grow a line; nothing truncates and no string changes.
    @Test
    fun `the home system in a Slide Over window`() {
        capture(width = 320, height = 860, uiState = homeSystemUiState, name = "galaxy_home_system_slide_over")
    }

    @Test
    fun `an unsurveyed system in a Slide Over window`() {
        capture(width = 320, height = 620, uiState = unsurveyedSystemUiState, name = "galaxy_unsurveyed_slide_over")
    }

    @Test
    fun `every verdict in a Slide Over window`() {
        capture(width = 320, height = 900, uiState = everyVerdictUiState, name = "galaxy_every_verdict_slide_over")
    }
