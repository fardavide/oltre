...
- **One module per client feature.** New feature = new Gradle module under `client/`
  (`:client:<feature>`), depending on `core` + `:client:design` only. `:client:shell` is the
  only module that sees all features; features never depend on each other — shared needs go
  down into `core` or `:client:design`.
- **New module checklist:** add to `settings.gradle.kts`; copy the target set from
  `:client:design`; namespace `dev.fardavide.oltre.client.<feature>`; wire into `:client:shell`.
