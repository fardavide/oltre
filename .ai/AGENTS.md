...
`.ai/AGENTS.md` is the canonical instruction file, exposed through the root `AGENTS.md`,
`CLAUDE.md`, and `GEMINI.md` entry points. Reusable workflows and knowledge live in
`.ai/skills/<name>/SKILL.md`; `.ai/skills`, `.agents/skills`, and `.gemini/skills` are native
adapters to that same flat skill tree. Command-like workflows are skills too, with a portable
description plus Claude's `when_to_use` and `user-invocable: true` metadata.

## Read first

- `.ai/docs/brief.md` — distilled brief; links the Notion design page. **Notion is the origin,
  not the authority** (Davide, 2026-08-16: *"Notion stuff is now very ancient"*) — the decision
  sheets in `.ai/docs/` and his live calls govern, and **no Notion number is a ceiling**. Never
  block work on one; if Notion is the only source for something, it is unanswered, so ask.
- `.ai/docs/architecture.md` + `.ai/docs/decisions.md` — before any non-trivial change.
- `.ai/docs/balance-log.md` — before touching any balance number: what was already tried,
  what playing it felt like, what is still open. Add a round there whenever tuning lands.
- `.ai/rules/session-roles.md` — **which session you are and what you may touch.** A cloud
  session may not write UI; hand-offs between sessions are ready-to-paste prompts.
- `.ai/docs/galaxy-sheet.md` — the settled design for slices 4 and 5 (the galaxy). Read it
  before writing a line of generation; it is the design, not a proposal.
- `.ai/docs/adaptation-sheet.md` — the settled design for the three adaptation ladders: why they
...
Invariants (raise, don't work around — full list in `.ai/docs/brief.md`):
...
its shared scheme. Never hand-edit `project.pbxproj`. See `.ai/docs/decisions.md`.
